...
-   Relevant Files Variables:
    ...
    1.  Basic_Stats
        ...
        2.  Age/Birthday (use the one with less NA?), Height, Weight
            1.  Age is removed because its calculating 2016-Birthday
            2.  Calculate Debut Age for Debut Year Statistics
        3.  Current Status, Current Team, Position (82% NA)
        4.  Experience calculated from Years Played, Years Played (18% null)
            1.  ***Survival Time:** Experience (Outcome)*
...
```{r}
raw_player <- read_csv(here("data", "Basic_Stats.csv"), show_col_types = FALSE)
raw_qb_logs <- read_csv(here("data", "Game_Logs_Quarterback.csv"), show_col_types = FALSE)
# very similar data as qb logs, but qb has a bit more 
raw_passes <- read_csv(here("data", "Career_Stats_Passing.csv"), show_col_types = FALSE)
...
# to get stats only for debut year
qb_first_year <- qb_regular |>
  group_by(`Player Id`) |>
  filter(Year == min(Year)) |>
  ungroup()
qb_first_year

# to get stats only for first 3 years 
qb_3_years <- qb_regular |> 
  group_by(`Player Id`) |>
...
qb_career_summary <- qb_regular |>
  ...
  mutate(
    ...
    bigT = 2016 - First_Year, # first year is debut year 
    
    # natural censoring: the last year is 2016 
    # Adjust this threshold based on your data's latest year
    # if retired or not (0/1)
    Event = if_else(Last_Year >= 2016, 0, 1),
...
# debut year stats
qb_debut_summary <- qb_first_year |>   
  select(-c(Week, `Passer Rating`)) |>
  mutate(across(all_of(variables), as.numeric)) |>
  group_by(`Player Id`, Name) |>
  
  summarise(
    Debut_Year_Games = sum(`Games Played` == 1, na.rm = TRUE),
    Last_Year  = max(Year, na.rm = TRUE),
  ...
  mutate(
    Debut_TD_INT = `TD Passes` / pmax(Ints, 1), # row wise math
    Debut_RUN_PASS = `Rushing Yards` / pmax(`Passing Yards`, 1),
    Event = if_else(Last_Year >= 2016, 0, 1))

# first 3 years 
qb_3year_summary <- qb_3_years |>   
  select(-c(Week, `Passer Rating`)) |>
  mutate(across(all_of(variables), as.numeric)) |>
  group_by(`Player Id`, Name) |>
  
  summarise(
    Three_Year_Games = sum(`Games Played` == 1, na.rm = TRUE),
    Last_Year  = max(Year, na.rm = TRUE),
  ...
  mutate(
    Three_Year_TD_INT = `TD Passes` / pmax(Ints, 1), # row wise math
    Three_Year_RUN_PASS = `Rushing Yards` / pmax(`Passing Yards`, 1),
    Event = if_else(Last_Year >= 2016, 0, 1))
...
qb_combined1 <- qb_career_summary |>
  ...
  # Age is their age currently in the year of 2016
         # 2016 - First Year calculates time since they debuted and 
         # subtract these years to get debut age  
  mutate(Debut_Age = Age - (2016 - First_Year))
qb_combined1

# all say same thing: "total_seasons", "time", "Experience", "career_length", "Years Played"

# reorder 
order <- c("Player Id", "Name.x", "Event", "Debut_Age", "Height (inches)", "Weight (lbs)",
...
qb_combined3 <- qb_combined2 |>
  ...
  select(-c("Passing Yards", "Sacks", "TD Passes", "Ints", "Rushing Yards", 
...
### Instead of Using Career Totals, Use Debut Year Stats and 3 Year Stats

[**Debut Year Statistics:**]{.underline}

```{r}
# keep everything in QB Logs
qb_debut_combined1 <- qb_debut_summary |>
  left_join(player_clean, by = "Player Id")
qb_debut_combined1

# all say same thing: "total_seasons", "time", "Experience", "career_length", "Years Played"
colnames(player_clean)
colnames(qb_debut_summary)
colnames(qb_3year_summary)

# reorder 
order <- c("Player Id", "Name.x", "Event", "Debut_Year_Games", "Height (inches)", 
           "Weight (lbs)","Passes Completed", "Passes Attempted", "Passing Yards", 
           "Sacks", "Ints", "TD Passes", "Rushing Yards", "Debut_TD_INT", "Debut_RUN_PASS")

qb_debut_combined2 <- qb_debut_combined1 |>
  select(all_of(order)) |>
  rename(Name = Name.x,
         Retired = Event) |>
  mutate(Debut_Completion_Percentage = `Passes Completed`/`Passes Attempted`)
         
qb_debut_combined2

# redued around 150 rows - should we keep or remove? 
qb_debut_combined3 <- qb_debut_combined2 |>
  mutate(
    Debut_Yards_per_game = `Passing Yards` / Debut_Year_Games,
    Debut_Sacks_per_game = Sacks / Debut_Year_Games,
    Debut_TD_per_game   = `TD Passes` / Debut_Year_Games,
    Debut_Ints_per_game  = Ints / Debut_Year_Games,
    Debut_Rush_per_game = `Rushing Yards`/ Debut_Year_Games) |>
  select(-c("Passing Yards", "Sacks", "TD Passes", "Ints", "Rushing Yards", 
...
safe_write_csv(qb_debut_combined3, here("nfl", "data","cleaned", "3_cleaned_nfl_debut.csv"))
```

[**Three Year Statistics:**]{.underline}

```{r}
# keep everything in QB Logs
qb_3year_combined1 <- qb_3year_summary |>
  left_join(player_clean, by = "Player Id")
qb_3year_combined1

# reorder 
order <- c("Player Id", "Name.x", "Event", "Three_Year_Games", "Height (inches)", 
           "Weight (lbs)","Passes Completed", "Passes Attempted", "Passing Yards", 
           "Sacks", "Ints", "TD Passes", "Rushing Yards", 
           "Three_Year_TD_INT", "Three_Year_RUN_PASS")

qb_3year_combined2 <- qb_3year_combined1 |>
  select(all_of(order)) |>
  rename(Name = Name.x,
         Retired = Event) |>
  mutate(Three_Year_Completion_Percentage = `Passes Completed`/`Passes Attempted`)
         
qb_3year_combined2

# redued around 150 rows - should we keep or remove? 
qb_3year_combined3 <- qb_3year_combined2 |>
  mutate(
    Three_Year_Yards_per_game = `Passing Yards` / Three_Year_Games,
    Three_Year_Sacks_per_game = Sacks / Three_Year_Games,
    Three_Year_TD_per_game   = `TD Passes` / Three_Year_Games,
    Three_Year_Ints_per_game  = Ints / Three_Year_Games,
    Three_Year_Rush_per_game = `Rushing Yards`/ Three_Year_Games) |>
  select(-c("Passing Yards", "Sacks", "TD Passes", "Ints", "Rushing Yards", 
            "Passes Completed", "Passes Attempted"))
            #"Total_Games", "Experience"))
...
safe_write_csv(qb_3year_combined3, here("nfl", "data","cleaned", "4_cleaned_nfl_3year.csv"))

```

[**Compare Empty Columns of Debut Year vs Three Year:**]{.underline}

```{r}
empty_columns2 <- colSums(qb_debut_combined3 == 0, na.rm = TRUE)
empty_columns2

empty_columns3 <- colSums(qb_3year_combined3 == 0, na.rm = TRUE)
empty_columns3

```

Most drastic difference in missingness is in the TD_INT ratio, where DEBUT has around 290 empty rows versus 198 empty rows. In all of the 3 year data, there is less missing data.

[**COMBINE OF ALL DATA: career, debut year, 3 year**]{.underline}

```{r}
all_data <- cbind(qb_combined3, qb_debut_combined3, qb_3year_combined3)

cols_keep <- c("Player Id", "Name", "Retired", "Debut_Age",
               "Height (inches)", "Weight (lbs)", "First_Year", "Experience",
               "Total_Games", "Debut_Year_Games", "Three_Year_Games",
               "TD_INT", "Debut_TD_INT", "Three_Year_TD_INT",
               "RUN_PASS", "Debut_RUN_PASS", "Three_Year_RUN_PASS",
               "Completion_Percentage", "Debut_Completion_Percentage",
               "Three_Year_Completion_Percentage",
               "Yards_per_game", "Debut_Yards_per_game", "Three_Year_Yards_per_game",
               "Sacks_per_game", "Debut_Sacks_per_game", "Three_Year_Sacks_per_game",
               "TD_per_game", "Debut_TD_per_game", "Three_Year_TD_per_game",
               "Ints_per_game", "Debut_Ints_per_game", "Three_Year_Ints_per_game", 
               "Rush_per_game", "Debut_Rush_per_game", "Three_Year_Rush_per_game")

all_data1 <- all_data[,cols_keep] |>
  rename(Player_ID = `Player Id`,
         Career_Time = Experience,
         Career_Games = Total_Games,
         Career_Run_Pass = RUN_PASS,
         Debut_Run_Pass = Debut_RUN_PASS,
         Three_Year_Run_Pass = Three_Year_RUN_PASS,
         Career_Completion_Percentage = Completion_Percentage,
         Career_Yards_PG = Yards_per_game, 
         Debut_Yards_PG = Debut_Yards_per_game,
         Three_Year_Yards_PG = Three_Year_Yards_per_game, 
         Career_Sacks_PG = Sacks_per_game,
         Debut_Sacks_PG = Debut_Sacks_per_game,
         Three_Year_Sacks_PG = Three_Year_Sacks_per_game,
         Career_TD_PG = TD_per_game,
         Debut_TD_PG = Debut_TD_per_game,
         Three_Year_TD_PG = Three_Year_TD_per_game, 
         Career_Ints_PG = Ints_per_game,
         Debut_Ints_PG = Debut_Ints_per_game,
         Three_Year_Ints_PG = Three_Year_Ints_per_game, 
         Career_Rush_PG = Rush_per_game,
         Debut_Rush_PG = Debut_Rush_per_game,
         Three_Year_Rush_PG = Three_Year_Rush_per_game)
...
```{r}
safe_write_csv(all_data1, here("nfl", "data","cleaned", "5_ALL_DATA.csv"))
```
...
```{r}
colnames(qb_debut_combined3)
debut_vars<- c("Debut_TD_INT", "Debut_RUN_PASS", "Debut_Completion_Percentage",
               "Debut_Yards_per_game", "Debut_Sacks_per_game","Debut_Rush_per_game")

# set up a 2 x 3 grid to display all histograms together 
par(mfrow = c(2,3))
#par(mfrow = c(3, 3))

for (i in 1:length(debut_vars)) { #game_stats
  current = qb_debut_combined3[[debut_vars[i]]] # get rows of that variable
  hist(current,
       xlab = debut_vars[i], 
       main = paste("Histogram of", debut_vars[i]),
       col = "lightblue",
       freq = FALSE)
}
...
```{r}
colnames(qb_3year_combined3)
three_year_vars<- c("Three_Year_TD_INT", "Three_Year_RUN_PASS",
                    "Three_Year_Completion_Percentage",
                    "Three_Year_Yards_per_game", "Three_Year_Sacks_per_game",
                    "Three_Year_Rush_per_game")

# set up a 2 x 3 grid to display all histograms together 
par(mfrow = c(2,3))
#par(mfrow = c(3, 3))

for (i in 1:length(three_year_vars)) { #game_stats
  current = qb_3year_combined3[[three_year_vars[i]]] # get rows of that variable
  hist(current,
       xlab = three_year_vars[i], 
       main = paste("Histogram of", three_year_vars[i]),
       col = "lightblue",
       freq = FALSE)
}
...
Three Year might be more accurate to use since it has a significantly less number of missing values. This can cause certain values to inflate in the debut data.
...
```{r}
ggplot(qb_debut_combined3, aes(x = factor(Retired), y = Debut_TD_INT)) +
  geom_violin(fill = "lightpink") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
  labs(x = "Retired", y = "TD_INT RATIO")

ggplot(qb_debut_combined3, aes(x = factor(Retired), y = Debut_Yards_per_game)) +
  geom_violin(fill = "lightpink") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
  labs(x = "Retired", y = "Passing Yards Per Game")

ggplot(qb_debut_combined3, aes(x = factor(Retired), y = Debut_TD_per_game)) +
  geom_violin(fill = "lightpink") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
  labs(x = "Retired", y = "Touchdowns Per Game")

ggplot(qb_debut_combined3, aes(x = factor(Retired), y = Debut_Ints_per_game)) +
  geom_violin(fill = "lightpink") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
...
```{r}
ggplot(qb_3year_combined3, aes(x = factor(Retired), y = Three_Year_TD_INT)) +
  geom_violin(fill = "lightgreen") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
  labs(x = "Retired", y = "TD_INT RATIO")

ggplot(qb_3year_combined3, aes(x = factor(Retired), y = Three_Year_Yards_per_game)) +
  geom_violin(fill = "lightgreen") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
  labs(x = "Retired", y = "Passing Yards Per Game")

ggplot(qb_3year_combined3, aes(x = factor(Retired), y = Three_Year_TD_per_game)) +
  geom_violin(fill = "lightgreen") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
  labs(x = "Retired", y = "Touchdowns Per Game")

ggplot(qb_3year_combined3, aes(x = factor(Retired), y = Three_Year_Ints_per_game)) +
  geom_violin(fill = "lightgreen") +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") +
  labs(x = "Retired", y = "Interceptions Per Game")
```
